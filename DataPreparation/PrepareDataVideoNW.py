...
def PrepareData(input_dir,features_dir,output_dir):
   ...
   for partition_dir in os.listdir(depression_dir):
      ...
      for file_name in os.listdir(depression_partition):
         ...
         features_file = features_dir + '/' + file_id 
         ...
         numpy.savetxt(features_save_file,features,fmt='%f',delimiter=',')


if __name__ == "__main__":
   if len(sys.argv) != 4:
      print "Incorrect usage \
            PrepareData.py InputDir FeatDir OutputDir"
   else: 
      input_dir = sys.argv[1] # the Labels directory supplied by organizers
      features_dir = sys.argv[2] # the features directory supplied by organizers
      output_dir = sys.argv[3]
      if not os.path.exists(output_dir):
         os.makedirs(output_dir) 
      PrepareData(input_dir,features_dir,output_dir)
