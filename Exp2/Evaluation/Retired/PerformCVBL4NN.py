...
def GetListMinMax(train_bl1_features):
   list_min_max = []   
   for i in range(train_bl1_features.shape[1]):
      cur_feat_vect = train_bl1_features[:,i]
      min_vect = numpy.amin(cur_feat_vect)
      max_vect = numpy.amax(cur_feat_vect)
      min_max = [min_vect, max_vect]
      list_min_max.append(min_max)
   return list_min_max
   
def TrainNN(train_bl1_features,train_labels_matrix,dev_bl1_features,test_bl1_features):
   # get list of min-max
   list_min_max = GetListMinMax(train_bl1_features)
   net = nl.net.newff(list_min_max,[10,3])
   error = net.train(train_bl1_features, numpy.array(train_labels_matrix), epochs=500, show=100, goal=0.02)
   dev_out = net.sim(dev_bl1_features)
   test_out = net.sim(test_bl1_features)

   return dev_out, test_out
...
def PerformCV():
   ...
   for i in range(count_splits):
      ...
      # get output of dev and test
      [dev_bl1_predict,test_bl1_predict] = TrainNN(train_bl1_features,train_labels_matrix,dev_bl1_features,test_bl1_features) 
