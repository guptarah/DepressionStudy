...
def PerformCV():
   ...
   all_test_ground_truth, all_test_fusion_output, all_test_b2_output, all_test_b3_output, all_test_b4_output, all_test_combined_output= \
   numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3))
   all_dev_ground_truth, all_dev_fusion_output, all_dev_b2_output, all_dev_b3_output, all_dev_b4_output, all_dev_combined_output= \
   numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3)), numpy.zeros((0,3))
   ...
   for i in range(count_splits):
      ...
      train_audio_features = train_scaled_features_audio
      train_labels_l1 = train_labels_matrix[0:int(.8*num_train_frames),:]
      train_labels_l2 = train_labels_matrix[int(.8*num_train_frames):-1,:]
      train_audio_features_l1 = train_audio_features[0:int(.8*num_train_frames),:] 
      train_audio_features_l2 = train_audio_features[int(.8*num_train_frames):-1,:] 
      dev_audio_features = dev_scaled_features_audio
      test_audio_features = test_scaled_features_audio
      train_video_features = train_scaled_features_video
      train_video_features_l1 = train_video_features[0:int(.8*num_train_frames),:] 
      train_video_features_l2 = train_video_features[int(.8*num_train_frames):-1,:] 
      dev_video_features = dev_scaled_features_video
      test_video_features = test_scaled_features_video
   

      # first layer
      audio_regr_l1 = linear_model.LinearRegression(n_jobs=8)
      audio_regr_l1.fit(train_audio_features_l1,train_labels_l1)
      train_audio_predict_l1 = audio_regr_l1.predict(train_audio_features_l2)
      ...
      train_video_predict_l1 = video_regr_l1.predict(train_video_features_l2)
      ...
      # second layer
      fusion_regr_l2 = linear_model.LinearRegression(n_jobs=8)
      train_fusion_features = numpy.concatenate((train_audio_predict_l1,train_video_predict_l1),axis=1)
      dev_fusion_features = numpy.concatenate((dev_audio_predict_l1,dev_video_predict_l1),axis=1)
      test_fusion_features = numpy.concatenate((test_audio_predict_l1,test_video_predict_l1),axis=1)
      fusion_regr_l2.fit(train_fusion_features,train_labels_l2)
      dev_fusion_predict = fusion_regr_l2.predict(dev_fusion_features)
      test_fusion_predict = fusion_regr_l2.predict(test_fusion_features)

      all_test_ground_truth = numpy.concatenate((all_test_ground_truth,test_labels_matrix),axis=0) 
      all_test_fusion_output = numpy.concatenate((all_test_fusion_output,test_fusion_predict),axis=0) 

      all_dev_ground_truth = numpy.concatenate((all_dev_ground_truth,dev_labels_matrix),axis=0) 
      all_dev_fusion_output = numpy.concatenate((all_dev_fusion_output,dev_fusion_predict),axis=0) 

      # Printing results
      print 'Dev set results'
      PrintResults(all_dev_ground_truth,all_dev_fusion_output)
     
      print ' ' 
      print 'Test set results'
      PrintResults(all_test_ground_truth,all_test_fusion_output)
