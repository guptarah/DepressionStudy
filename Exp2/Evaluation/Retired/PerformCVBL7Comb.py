...
def PerformCV():
   ...
   for i in range(count_splits):
      ...
      dev_results = numpy.zeros((11,1))
      dev_combined_filt, test_combined_filt = numpy.zeros(dev_combined.shape), numpy.zeros(test_combined.shape) 
      for alpha in range(1,11):
         win_len = 10*alpha
         conv_filt = (1.0/win_len)*numpy.ones((1,win_len))[0,:]
         for dim in range(3):
            cur_ts = numpy.squeeze(numpy.array(dev_combined[:,dim].T))
            dev_combined_filt[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
         dev_results[alpha] = PrintResults(dev_labels_matrix,dev_combined_filt)                        
      best_alpha = numpy.argmax(dev_results,axis=0) 
      win_len = 10*best_alpha
      print 'chose win len: ', win_len
      conv_filt = (1.0/win_len)*numpy.ones((1,win_len))[0,:]
      for dim in range(3):
         cur_ts = numpy.squeeze(numpy.array(dev_combined[:,dim].T))
         dev_combined_filt[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
         cur_ts = numpy.squeeze(numpy.array(test_combined[:,dim].T))
         test_combined_filt[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')

      all_dev_fused_output = numpy.concatenate((all_dev_fused_output,dev_combined_filt),axis=0) 
      all_test_fused_output = numpy.concatenate((all_test_fused_output,test_combined_filt),axis=0) 
