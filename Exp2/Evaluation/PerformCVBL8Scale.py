...
#      # low pass filtering the output
#      win_len = 200 
#      conv_filt = (1.0/win_len)*numpy.ones((1,win_len))[0,:]
#      for dim in range(3):
#         cur_ts = numpy.squeeze(numpy.array(dev_audio_predict_l1[:,dim].T))
#         dev_audio_predict_l1[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
#         cur_ts = numpy.squeeze(numpy.array(dev_video_predict_l1[:,dim].T))
#         dev_video_predict_l1[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
#         cur_ts = numpy.squeeze(numpy.array(test_audio_predict_l1[:,dim].T))
#         test_audio_predict_l1[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
#         cur_ts = numpy.squeeze(numpy.array(test_video_predict_l1[:,dim].T))
#         test_video_predict_l1[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
   ...
   # Final results also after filtering
   win_len = 300
   print 'winlen: ', win_len
   conv_filt = (1.0/win_len)*numpy.ones((1,win_len))[0,:]
   for dim in range(3):
      cur_ts = numpy.squeeze(numpy.array(all_test_audio_output[:,dim].T))
      all_test_audio_output[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
      cur_ts = numpy.squeeze(numpy.array(all_test_video_output[:,dim].T))
      all_test_video_output[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
      cur_ts = numpy.squeeze(numpy.array(all_test_fused_output[:,dim].T))
      all_test_fused_output[:,dim] = numpy.convolve(cur_ts,conv_filt,mode='same')
